...
def _synchronize() -> tuple[bool, str | None]:
    try:
        warp = importlib.import_module("warp")
...
def run_warp_timing(
    ...
    try:
        ...
        warp_data = _make_warp_data(mjw, warp_model, mj_model, mj_data, nworld, nconmax, naconmax, njmax)
        ...
        synchronized, sync_warning = _synchronize()
        warmup_start = time.perf_counter()
        for _ in range(warmup_steps):
            mjw.step(warp_model, warp_data)
        synchronized_after_warmup, sync_warning_after = _synchronize()
...
def availability_payload(availability: WarpAvailability) -> dict[str, Any]:
    ...
    return {
        ...
        "slurm": {key: value for key in slurm_keys if (value := __import__("os").environ.get(key)) is not None},
    }
